<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:53:"C:\wamp64\www/application/home\view\index\single.html";i:1493433367;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
<title>single</title>
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
			<div class="show-top-grids">
				<div class="col-sm-8 single-left">
					...
					<img src="<?php echo $book['image_path']; ?>" alt="" style="height: 350px;float: left;" />
					<div class="load_more" style="height: 230px;">	
						<ul style="width: 450px;margin-left: 260px;">
					...
					<div style="font-size: 12px; float: left;margin-left: 20px;">
						<span>总点击：<?php echo $book['browse_count']; ?></span>&nbsp;|&nbsp;<span>总收藏：<?php echo $book['collect_count']; ?></span>
					</div>
						<br /><br />
						<input type="button" class="btn btn-primary" style="margin-left: 20px;" id="start_read" name="start_read" value="点击阅读">
						&nbsp;&nbsp;&nbsp;
						<input type="button"  class="btn btn-info" onclick="add_collect(<?php echo $book['id']; ?>)" name="collect" value="加入收藏">
					                 <div class="chapterlist_box" style="margin-top: 50px;">
                    <b class="tl"></b><b class="tr"></b><b class="bl"></b><b class="br"></b>
                    <!--章节-->
                    <ul class="cf">
                       <?php if(is_array($list) || $list instanceof \think\Collection): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$chapter): $mod = ($i % 2 );++$i;?>
                        <li>
                        <a  onclick="read(<?php echo $chapter['zid']; ?>);"  title="<?php echo $chapter['zid']; ?>章"  target="_blank"  class="colorful">
                        <em></em>第 <?php echo $chapter['zid']; ?> 章</a>
                        </li>
                    ...
                    <script src="__Js__/layer/layer.js"></script>
							<script>
								function read(zid){
									var book = <?php echo $book['id']; ?>;
									var yid=1;

									$.ajax({
								        url:"<?php echo url('read'); ?>",
								        type:'POST', //GET
								        ...
								        success:function(data,textStatus,jqXHR){
								            if(data.code==1){
								                layer.msg(data.msg, {icon: 6,time:1500,shade: 0.1}, function(index){
								                    ...
								                    window.location.href = "<?php echo url('shows'); ?>"+"?book="+book+"&chapter="+zid+"&page="+yid;
								                });
								            }else{
				...
				<div class="col-md-4 single-right" style="width: 320px;margin-left: 50px;">
					<h3>作者信息</h3>
					<div class="single-grid-right">
						<div class="single-right-grids">
							<div class="col-md-4 single-right-grid-left">
								<a href="<?php echo url('/home/personal/usercomics',array('id'=>$auther['id'])); ?>"><img src="<?php echo $auther['image']; ?>" alt="" /></a>
							</div>
							<div class="col-md-8 single-right-grid-right">
								<a href="<?php echo url('/home/personal/usercomics',array('id'=>$auther['id'])); ?>" class="title"> <?php echo $auther['nickname']; ?></a>
								<p class="author"><a href="#" class="author"><?php echo $auther['nickname']; ?></a></p>
								<input type="button" class="btn btn-info" name="concern" onclick="add_con(<?php echo $auther['id']; ?>)" value="关注">
							</div>
							<div class="clearfix"> </div>
						</div>
						<h3>作者其他作品</h3>
						<?php if(is_array($other) || $other instanceof \think\Collection): $i = 0; $__LIST__ = $other;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$other): $mod = ($i % 2 );++$i;if($other['id']!=$book['id']): ?>
						<div class="single-right-grids">
							<div class="col-md-4 single-right-grid-left">
								<a href="<?php echo url('single',array('book'=>$other['id'])); ?>"><img src="<?php echo $other['image_path']; ?>" alt="" /></a>
							</div>
							<div class="col-md-8 single-right-grid-right">
								<a href="<?php echo url('single',array('book'=>$other['id'])); ?>" class="title"> <?php echo $other['title']; ?></a>
								<p class="author"><a href="#" class="author"><?php echo $auther['nickname']; ?></a></p>
								<p class="views"><?php echo $other['browse_count']; ?> views </p>
							</div>
							<div class="clearfix"> </div>
						</div>
						...
						<script src="__Js__/layer/layer.js"></script>
							<script>
								$(document).ready(function () {
									...
									$('#start_read').click(function(event) {
										/* Act on the event */
										
										window.location.href="<?php echo url('shows',array('book'=>$book['id'],'chapter'=>1)); ?>";
									});									
								});
								function add_collect(id){
									$.ajax({
								        url:"<?php echo url('add_collect'); ?>",
								        type:'POST', //GET
								        async:true,    //或false,是否异步
								        data:{
								            id:id
								...
								function add_con(tuid){
									$.ajax({
								        url:"<?php echo url('add_con'); ?>",
								        type:'POST', //GET
								        async:true,    //或false,是否异步
								        data:{
								        	//username:username,
